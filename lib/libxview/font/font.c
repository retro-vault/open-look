/*
 * Implements the font routines used by the XView font module.
 *
    ...
    struct font_return_attrs *attrs;
{
    ...
    if (fs)
        snprintf(new_name, sizeof(new_name), "%s-%d", name, desired_scale);
    else
        snprintf(new_name, sizeof(new_name), "%s-*-%d-*-*-*-*-*-*", name,
                 (10*desired_scale));
    ...
    struct font_return_attrs *attrs;
{
    ...
    snprintf(new_name, sizeof(new_name), "%s-*-%d-*-*-*-*-%s-%s", name,
             (10*desired_scale),
