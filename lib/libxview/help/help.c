/*
 * Implements the help routines used by the XView help module.
 *
    ...
    Event          *client_event;
{
    ...
    if (length > (int)(sizeof(client_name) / sizeof(client_name[0]) - 1 -
		       STRLEN(XV_MSG(": Help"))))
	length = (int)(sizeof(client_name) / sizeof(client_name[0]) - 1 -
		       STRLEN(XV_MSG(": Help")));

    STRNCPY(client_name, application_name, length);
    client_name[length] = 0;
    STRNCAT(client_name, XV_MSG(": Help"),
	    (sizeof(client_name) / sizeof(client_name[0])) -
	    STRLEN(client_name) - 1);
