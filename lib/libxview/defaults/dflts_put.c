/*
 * Implements the dflts put routines used by the XView defaults module.
 *
...
#include <view2/sun.h>		/* Define True, False, etc... */
#include <view2_private/hash.h>	/* Hash table definitions */
#include <view2_private/parse.h>/* Parsing routine */
#include <string.h>
#include <view2/defaults.h>
#include <view2_private/dflts_impl.h>
 ...
 * rearrange the defaults database so that From_Path_Name will internally be
 * next to To_Path_Name.  If Before_Flag is True, a call to
 * defaults_get_sibling(To_Path_Name) will return From_Path_Name. If
 * Before_Flag is False, a call to defaults_get_child(From_Path_Name) will
 * return To_Path_Name.
 */
void
...
/*
 * defaults_set_integer(path_name, value, status) will set Path_Name to
 ...
 * "/Fee/Fie/Fo" will be looked at.  This is used to permit individual
 * programs to permit overriding of defaults.  If Prefix is NULL, the prefix
 * will be cleared.
 */
void
...
/*
 * defaults_write_changed(file_name, status) will write out all of the
 * private database entries to File_Name.  Any time a database node is set it
 * becomes part of the private database.  Out_File is the string  name of the
 * file to create.  If File_Name is NULL, env var DEFAULTS_FILE will be used.
 */
void
    ...
    int             flag;	/* Control flag */
{
	...
	if ((file_name = getenv("DEFAULTS_FILE")) == NULL) {
		...
		(void) strcpy(file_name, xv_getlogindir());
	    }
 ...
 * Out_File.  Flag is 0, if the entire database is to be written out. Flag is
 * 1 if only the changed entries are to be written out.  Flag is 2, if only
 * differences from the master database are to be written out.
 */
static void
node_write1(out_file, node, flag)
    register FILE  *out_file;	/* Output file */
...
/*
 * set(path_name, new_value) will set Full_Name to type Node_Type with value
 * New_Value.
 */
static void
set(path_name, new_value, status)
...
defaults_special_mode()
{
    ...
    /*
     * Read in env var DEFAULTS_FILE to find out where database directories
     * are.
     */
    ...
    /*
     * Read in master database before private database so that node order is
     * preserved in master database order rather than private order.
     */
    ...
    register char  *string;	/* String to output */
{
	...
	if ((' ' <= chr) && (chr <= '~'))
	    ...
	    (void) fprintf(out_file, "%03o", chr & 0377);
    }
    putc('"', out_file);
}

/*
 * warn(format, arg1, arg2, arg3) will print out a warning message to the
 * console using Format, Arg1, Arg2 and Arg3.  This is all provided the
 * number of errors has not gotten too excessive.
 */
/* VARARGS1 */
void
