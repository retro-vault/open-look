...
static KeySym keycode_to_keysym(Display *dpy, KeyCode keycode, int index);
...
static KeySym
keycode_to_keysym(dpy, keycode, index)
    Display *dpy;
    KeyCode keycode;
    int index;
{
    ...
    keysyms = XGetKeyboardMapping(dpy, keycode, 1, &keysyms_per_keycode);
    if (keysyms) {
        if ((index >= 0) && (index < keysyms_per_keycode))
...
char		*qual_str;	/* return */
{
	 ...
	 * index 1.
	 */
        unmod_keysym = keycode_to_keysym(dpy, keycode, 0);
        shifted_ksym = keycode_to_keysym(dpy, keycode, 1);
...
XrmDatabase	db;
{
	...
	char funcname[128], resname[256];
	XrmValue value;
	char *strtype;

	*funcname = '\0';
	SSCANF( resourceString, "%*s%127s", funcname );

	/*
	 * Put resource name in multibyte buffer to pass to XrmGetResource()
	 */
	snprintf(resname, sizeof(resname),
		 "OpenWindows.MenuAccelerator.%s", funcname);
