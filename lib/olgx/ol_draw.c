...
int calc_add_ins(width, add_ins)
int width;
...

{
	...
	if (info->three_d)
	{
		...
		if (fill_in && width > 1 && height > 1)
		{
			...
			XFillRectangle(info->dpy, win,
						   (state & OLGX_INVOKED) ? info->gc_rec[OLGX_BG2]->gc : info->gc_rec[OLGX_BG1]->gc,
		...
		XDrawLines(info->dpy, win,
				   (state & OLGX_INVOKED) ? info->gc_rec[OLGX_BG3]->gc : info->gc_rec[OLGX_WHITE]->gc, point, 3, CoordModeOrigin);
		...
		if (info->three_d == OLGX_3D_MONO)
		{
			...
			XDrawLines(info->dpy, win,
					   (state & OLGX_INVOKED) ? info->gc_rec[OLGX_WHITE]->gc : info->gc_rec[OLGX_BG3]->gc,
	...
	else
	{
		...
		if (state & OLGX_ERASE)
			XFillRectangle(info->dpy, win, info->three_d ? info->gc_rec[OLGX_BG1]->gc : info->gc_rec[OLGX_WHITE]->gc,
...
int x, y, width, height, state;
{
	...
	if (state & OLGX_DEFAULT)
	{
		...
		XDrawRectangle(info->dpy, win, (info->three_d) ? info->gc_rec[OLGX_BG3]->gc : info->gc_rec[OLGX_BLACK]->gc,
...
int x, y, state;
{
	...
	XFillRectangle(info->dpy, win, (info->three_d) ? info->gc_rec[OLGX_BG2]->gc : info->gc_rec[OLGX_WHITE]->gc,
				   x, y, width, height);
	XFillPolygon(info->dpy, win, (info->three_d) ? info->gc_rec[OLGX_BG3]->gc : info->gc_rec[OLGX_BLACK]->gc,
	...
	if (state & OLGX_INVOKED)
	{
		...
		{
			XDrawLine(info->dpy, win, (info->three_d) ? info->gc_rec[OLGX_BLACK]->gc : info->gc_rec[OLGX_BLACK]->gc,
