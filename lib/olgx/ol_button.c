/*
 * ol_button.c: OPEN LOOK object drawing package Sun Microsystems, Inc.
 *
 * NOTES:
...
int state;
{
	...
	else
	{
		...
		if (info->three_d)
		{
			...
			{ /* if not transparent, actually draw the
			   * button */
	...
	if (label)
	{
		...
		if (state & OLGX_LABEL_IS_PIXMAP)
		{
			...
			olgx_draw_pixmap_label(info, win,
								   ...
								   (((Pixlabel *)label)->width > width) ? width : ((Pixlabel *)label)->width,
								   (height) ? ((((Pixlabel *)label)->height > height) ? height : ((Pixlabel *)label)->height)
											: Button_Height(info) - 2,
								   state);
		}
...

{
	...
	if (info->three_d)
	{
		...
		XDrawString(info->dpy, win, (state & OLGX_INVOKED) ? info->gc_rec[OLGX_BG3]->gc : info->gc_rec[OLGX_WHITE]->gc, x, y,
					string, 1);

		string[0] = PIXLABEL_BUTTON_UR;
		XDrawString(info->dpy, win, (state & OLGX_INVOKED) ? info->gc_rec[OLGX_BG3]->gc : info->gc_rec[OLGX_WHITE]->gc,
					x + width - VARHEIGHT_BUTTON_CORNER_DIMEN, y, string, 1);

		string[0] = PIXLABEL_BUTTON_LL;
		XDrawString(info->dpy, win, (state & OLGX_INVOKED) ? info->gc_rec[OLGX_WHITE]->gc : info->gc_rec[OLGX_BG3]->gc, x, y + height - VARHEIGHT_BUTTON_CORNER_DIMEN, string, 1);

		string[0] = PIXLABEL_BUTTON_LR;
		XDrawString(info->dpy, win, (state & OLGX_INVOKED) ? info->gc_rec[OLGX_WHITE]->gc : info->gc_rec[OLGX_BG3]->gc,
		...
		XDrawSegments(info->dpy, win, (state & OLGX_INVOKED) ? info->gc_rec[OLGX_BG3]->gc : info->gc_rec[OLGX_WHITE]->gc,
					  seg, 2);
		seg[0].x1 = x + VARHEIGHT_BUTTON_CORNER_DIMEN;
		...
		seg[1].y1 = y + VARHEIGHT_BUTTON_CORNER_DIMEN;
		seg[1].y2 = y + height - VARHEIGHT_BUTTON_CORNER_DIMEN;
		XDrawSegments(info->dpy, win, (state & OLGX_INVOKED) ? info->gc_rec[OLGX_WHITE]->gc : info->gc_rec[OLGX_BG3]->gc,
					  seg, 2);
	}
	else
	{
	...
	if (state & OLGX_DEFAULT)
	{
		...
		XDrawString(info->dpy, win, info->gc_rec[OLGX_BLACK]->gc, x + width - VARHEIGHT_BUTTON_CORNER_DIMEN, y, string, 1);

		string[0] = PIXLABEL_DEF_BUTTON_LL;
		XDrawString(info->dpy, win,
					info->gc_rec[OLGX_BLACK]->gc, x, y + height - VARHEIGHT_BUTTON_CORNER_DIMEN, string, 1);

		string[0] = PIXLABEL_DEF_BUTTON_LR;
		XDrawString(info->dpy, win,
					info->gc_rec[OLGX_BLACK]->gc, x + width - VARHEIGHT_BUTTON_CORNER_DIMEN, y + height - VARHEIGHT_BUTTON_CORNER_DIMEN, string, 1);
...
int state;
{
	...
	{ /* 3d */
		...
		olgx_draw_menu_mark(info, win, x + ((Abbrev_MenuButton_Width(info) - info->mm_width) >> 1),
...

{
	...
	{ /* draw 3-D */
		...
		olgx_draw_box(info, win, x, y, width, height,
					  (state & OLGX_SCROLL_BACKWARD) ? OLGX_INVOKED : OLGX_NORMAL, 0);
		olgx_draw_box(info, win, x + width, y, width, height,
					  (state & OLGX_SCROLL_FORWARD) ? OLGX_INVOKED : OLGX_NORMAL, 0);
		olgx_draw_menu_mark(info, win, x + arr_x, y + arr_y,
	...
	if (main_label)
	{
		...
		else if (state & OLGX_LABEL_IS_XIMAGE)
		{
			...
			olgx_draw_ximage_label(info, win,
								   ...
								   m_pos,
								   newy,
								   (ximlabel->width > text_width) ? text_width : ximlabel->width,
					...
					if (state & OLGX_LABEL_IS_WCS)
					{
						...
						highlight_length = XwcTextEscapement(info->textfontset,
															 (wchar_t *)((wchar_t *)string_label +
																		 hlabel->position),
															 1);
						highlightx_pos = m_pos + ((hlabel->position) ? string_width : 0);
					}
					else
					{ /* multibyte case */
				...
				{ /* single byte i18n case */
					...
					highlight_length = XTextWidth(TextFont_Struct(info),
												  (char *)string_label +
													  hlabel->position,
												  1);
					highlightx_pos = m_pos + ((hlabel->position) ? overall.width : 0);
				...
				if (Olgx_Flags(info) & OLGX_FONTSET)
				{
					...
					if (state & OLGX_LABEL_IS_WCS)
					{
						label_pixel_pos = m_pos +
										  ((ulabel->position) ? XwcTextEscapement(info->textfontset,
																				  (wchar_t *)string_label,
																				  ulabel->position)
															  : 0);
					...
					{
						label_pixel_pos = m_pos +
										  ((ulabel->position) ? XmbTextEscapement(info->textfontset,
																				  (char *)string_label,
																				  ulabel->position)
															  : 0);
...
int state;				 /* state of the actual object */
{
	...
	olgx_draw_accel_label_internal(info, win,
								   y +
									   ((height) ? ((height + info->button_height) / 2 + 1) : info->button_height) -
									   info->base_off,
...
int state;				 /* state of the actual object */
{
	...
	olgx_draw_accel_label_internal(info, win,
								   ...
								   y,
								   width - ((info->button_height > 20) ? info->base_off + 2 : info->base_off),
								   height,
								   main_label, m_pos,
								   qualifier_label, q_pos,
								   mark_type, mark_pos,
