...

{
	...
	if (state & OLGX_HORIZONTAL)
	{
		...
		tmp_dimension = (x + length - cable_offset - 2) - rectangles[1].x;
		...
		if (prop_length)
		{
			...
			XFillRectangles(ginfo->dpy, win, (ginfo->three_d) ? ginfo->gc_rec[OLGX_BG3]->gc : ginfo->gc_rec[OLGX_BLACK]->gc,
							&rectangles[0], 2);
		}
	...
	{ /* Vertical Scrollbar */
		...
		tmp_dimension = (y + length - cable_offset - 2) - rectangles[1].y;
...
int state;
{
	...
	if ((state & OLGX_ERASE))
		XClearArea(info->dpy, win,
				   x - 1, y - 1,
				   (state & OLGX_HORIZONTAL) ? (sbheight + 2) : ((ScrollbarElevator_Width(info)) + 2),
				   (state & OLGX_VERTICAL) ? (sbheight + 2) : ((ScrollbarElevator_Width(info)) + 2), 0);

	XFillRectangle(info->dpy, win, info->three_d ? info->gc_rec[OLGX_BG1]->gc : info->gc_rec[OLGX_WHITE]->gc, x, y,
				   (state & OLGX_HORIZONTAL) ? (sbheight - 1) : ((ScrollbarElevator_Width(info)) - 1),
				   (state & OLGX_VERTICAL) ? (sbheight - 1) : ((ScrollbarElevator_Width(info)) - 1));
	...
	{ /* 3d begins */
		...
		else if (state & OLGX_SCROLL_FORWARD)
		{
			...
			string[0] = (state & OLGX_VERTICAL) ? VERT_SB_BOTBOX_FILL : HORIZ_SB_RIGHTBOX_FILL;
		...
		if (state & OLGX_INACTIVE)
			olgx_stipple_rect(info, win, x, y,
							  (state & OLGX_HORIZONTAL) ? (sbheight + 2) : ((ScrollbarElevator_Width(info)) + 2),
							  (state & OLGX_VERTICAL) ? (sbheight + 2) : ((ScrollbarElevator_Width(info)) + 2));
	}
	...
	else
	{
		...
		{ /* abbrev scroll bar */
			...
			if (state & OLGX_SCROLL_BACKWARD)
				initial = (state & OLGX_HORIZONTAL) ? OLG_HSB_REDUCED_ELEVATOR_LINE_BACKWARD : OLG_VSB_REDUCED_ELEVATOR_LINE_BACKWARD;
				...
				initial = (state & OLGX_HORIZONTAL) ? OLG_HSB_REDUCED_ELEVATOR : OLG_VSB_REDUCED_ELEVATOR;
		}
		else
		{ /* normal scrollbar */

			if (state & OLGX_SCROLL_BACKWARD)
				initial = (state & OLGX_HORIZONTAL) ? OLG_HSB_ELEVATOR_LINE_BACKWARD : OLG_VSB_ELEVATOR_LINE_BACKWARD;

			else if (state & OLGX_SCROLL_NO_BACKWARD)
				initial = (state & OLGX_HORIZONTAL) ? HORIZ_SB_NO_BACK_OUTLINE : VERT_SB_NO_BACK_OUTLINE;

			else if (state & OLGX_SCROLL_NO_FORWARD)
				initial = (state & OLGX_HORIZONTAL) ? HORIZ_SB_NO_FWD_OUTLINE : VERT_SB_NO_FWD_OUTLINE;

			else if (state & OLGX_INACTIVE)
				initial = (state & OLGX_HORIZONTAL) ? HORIZ_SB_INACTIVE_OUTLINE : VERT_SB_INACTIVE_OUTLINE;

			else if (state & OLGX_SCROLL_FORWARD)
				initial = (state & OLGX_HORIZONTAL) ? OLG_HSB_ELEVATOR_LINE_FORWARD : OLG_VSB_ELEVATOR_LINE_FORWARD;

			else if (state & OLGX_SCROLL_ABSOLUTE)
				initial = (state & OLGX_HORIZONTAL) ? OLG_HSB_ELEVATOR_ABSOLUTE : OLG_VSB_ELEVATOR_ABSOLUTE;

			else /* default - normal -uninvoked scrollbar */
				initial = (state & OLGX_HORIZONTAL) ? OLG_HSB_ELEVATOR : OLG_VSB_ELEVATOR;
		}
