...
int x, y, state;
{
	...
	if (state & OLGX_HORIZONTAL)
	{
		if (info->three_d)
		{
			...
			XDrawText(info->dpy, win, (state & OLGX_INVOKED) ? info->gc_rec[OLGX_BG3]->gc : info->gc_rec[OLGX_WHITE]->gc,
					  x, y, &item, 1);

			string[0] = HORIZ_SLIDER_CONTROL_LR;
			XDrawText(info->dpy, win, (state & OLGX_INVOKED) ? info->gc_rec[OLGX_WHITE]->gc : info->gc_rec[OLGX_BG3]->gc,
					  x, y, &item, 1);

			string[0] = HORIZ_SLIDER_CONTROL_FILL;
			XDrawText(info->dpy, win, (state & OLGX_INVOKED) ? info->gc_rec[OLGX_BG2]->gc : info->gc_rec[OLGX_BG1]->gc,
					  x, y, &item, 1);
		}
		else
		{
			...
			string[0] = HORIZ_SLIDER_CONTROL_FILL;
			XDrawText(info->dpy, win, (state & OLGX_INVOKED) ? info->gc_rec[OLGX_BLACK]->gc : info->gc_rec[OLGX_WHITE]->gc,
					  x, y, &item, 1);
		}
	}
	else
...
int state;
{
	...
	if (old_value < new_value)
	{
		...
		XDrawLine(info->dpy, win, info->three_d ? info->gc_rec[OLGX_BG2]->gc : info->gc_rec[OLGX_BLACK]->gc, xstart, y_slider_offset + 1,
				  xstart + xwidth - 1, y_slider_offset + 1);

		if (old_value < 4)
		{ /* Draw the leftendcap when necessary */
	...
	else
	{
		...
		XDrawLine(info->dpy, win, info->three_d ? info->gc_rec[OLGX_BG3]->gc : info->gc_rec[OLGX_BLACK]->gc,
				  xstart, y_slider_offset, xstart + xwidth - 1, y_slider_offset);

		XDrawLine(info->dpy, win, info->three_d ? info->gc_rec[OLGX_WHITE]->gc : info->gc_rec[OLGX_BLACK]->gc,
				  xstart, y_slider_offset + (info->se_height - 1), xstart + xwidth - 1,
				  y_slider_offset + (info->se_height - 1));

		XFillRectangle(info->dpy, win, info->three_d ? info->gc_rec[OLGX_BG2]->gc : info->gc_rec[OLGX_WHITE]->gc, xstart,
...
int state;
{
	...
	if (old_value > new_value)
	{
		...
		XDrawLine(info->dpy, win, info->three_d ? info->gc_rec[OLGX_BG2]->gc : info->gc_rec[OLGX_BLACK]->gc, x_slider_offset + 1, ystart,
	...
	else
	{
		...
		XDrawLine(info->dpy, win, info->three_d ? info->gc_rec[OLGX_BG3]->gc : info->gc_rec[OLGX_BLACK]->gc, x_slider_offset, ystart,
		...
		XFillRectangle(info->dpy, win, info->three_d ? info->gc_rec[OLGX_BG2]->gc : info->gc_rec[OLGX_WHITE]->gc, x_slider_offset + 1, ystart,
...

{
	...
	if (oldval < newval)
		XFillRectangle(info->dpy, win, info->gc_rec[OLGX_BLACK]->gc, x + oldval,
					   ...
					   (info->gauge_height == 13) ? info->se_height + 1 : info->se_height);

	else
		XFillRectangle(info->dpy, win, (info->three_d) ? info->gc_rec[OLGX_BG2]->gc : info->gc_rec[OLGX_WHITE]->gc,
...

{
	...
	XFillRectangle(info->dpy, win, info->gc_rec[OLGX_BLACK]->gc, x + left_offset,
				   y + value, (info->gauge_height == 13) ? info->se_height + 1 : info->se_height, width - value - left_offset - info->se_width - 1);
