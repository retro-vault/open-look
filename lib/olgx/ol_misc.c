/*
 * ol_misc.c: OPEN LOOK object drawing package Sun Microsystems, Inc.
 *
 * NOTES:
...
int x, y, type;
{
	...
	if (info->three_d)
	{
		...
		else
		{
			...
			string[0] = (type & OLGX_PUSHPIN_IN) ? PUSHPIN_IN_TOP : PUSHPIN_OUT_TOP;
			XDrawText(info->dpy, win, info->gc_rec[top_color]->gc, x, y, &item, 1);

			/* draw the lower right part */

			string[0] = (type & OLGX_PUSHPIN_IN) ? PUSHPIN_IN_BOTTOM : PUSHPIN_OUT_BOTTOM;
			XDrawText(info->dpy, win, info->gc_rec[bottom_color]->gc, x, y, &item, 1);

			string[0] = (type & OLGX_PUSHPIN_IN) ? PUSHPIN_IN_MIDDLE : PUSHPIN_OUT_MIDDLE;
			XDrawText(info->dpy, win, info->gc_rec[fill_color]->gc, x, y, &item, 1);
		}
	}
	else
	{ /* 2d */
		...
		else
		{
			...
			string[0] = (type & OLGX_PUSHPIN_IN) ? OLG_MENU_PIN_IN : OLG_MENU_PIN_OUT;
			XDrawText(info->dpy, win, info->gc_rec[OLGX_BLACK]->gc, x, y, &item, 1);
		}
	}
...
int x, y, state;
{
	...
	if (state & OLGX_CHECKED)
	{
		...
		XDrawText(info->dpy, win, info->three_d ? info->gc_rec[fill_color]->gc : info->gc_rec[OLGX_WHITE]->gc,
				  x, y, &item, 1);
	}
	else
	{
