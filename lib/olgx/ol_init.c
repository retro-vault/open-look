...

{
    ...
    info->gauge_endcapOffset = (info->gauge_height < 17) ? (info->gauge_height - 3) : 13;
...
#endif
{
    ...
    { /* not special */
        ...
        if (!info->three_d)
        {
            ...
            if (info->gc_rec[OLGX_TEXTGC_REV]->ref_count > 1)
            { /* GC is shared */
...

{
    ...
    {
        if ((cur->valuemask == valuemask) && (cur->depth == depth) && (gc_matches(cur, valuemask, values)))
        {
...
int gc_matches(GCrec, valuemask, values)
GC_rec *GCrec;
unsigned long valuemask;
XGCValues *values;
...

{
         ...
         cur != NULL; prev = cur, cur = cur->next)
    {
        ...
        if ((cur->valuemask == valuemask) && (gc_matches(cur, valuemask, values)))
        {
...

{
         ...
         prev = cur, cur = cur->next)
    {
        ...
        if ((cur->dpy == dpy) && (cur->screen == screen))
        {
...

{
    ...
    case OLGX_BG3:
        ...
        if (!info->three_d)
        {
            ...
            if (Olgx_Flags(info) & OLGX_FONTSET)
            {
                info->gc_rec[OLGX_TEXTGC_REV] = olgx_get_gcrec(perdispl_res_ptr, info->drawable[0],
    ...
    case OLGX_TEXTGC:
        ...
        values.foreground = info->pixvals[OLGX_BLACK];
        values.background = (info->three_d) ? info->pixvals[OLGX_BG1] : info->pixvals[OLGX_WHITE];
#ifdef OW_I18N
        if (Olgx_Flags(info) & OLGX_FONTSET)
        {
            info->gc_rec[OLGX_TEXTGC] = olgx_get_gcrec(perdispl_res_ptr, info->drawable[0],
    ...
    case OLGX_GREY_OUT:
        ...
        values.foreground = (info->three_d == OLGX_3D_COLOR) ? info->pixvals[OLGX_BG1] : info->pixvals[OLGX_WHITE];
        values.stipple = olgx_get_grey_stipple(perdispl_res_ptr);
    ...
    case OLGX_BUSYGC:
        ...
        values.background = info->pixvals[OLGX_BLACK];
        valuemask = (GCBackground | GCForeground | GCGraphicsExposures | GCFont | GCStipple | GCFillStyle);
...
int olgx_cmp_fonts(font_info1, font_info2)
...
#ifdef OW_I18N
int olgx_cmp_fontsets(fontset_info1, fontset_info2)
