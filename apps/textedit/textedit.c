 ...
 */
/*
 * Implements the TextEdit application used by the XView app suite.
 *
...
static void
textedit_refresh_current_directory(void)
{
	if (!getcwd(current_directory, sizeof(current_directory))) {
		(void)strncpy(current_directory, ".", sizeof(current_directory) - 1);
	...
	Attr_avlist	 attributes;
{
	...
	for (attrs = attributes; *attrs; attrs = attr_next(attrs)) {
		...
		switch (attr_string[0]) {
		  ...
		  case '.':
			    if (attr_string[1] != '\0')
				textedit_refresh_current_directory();
		    break;
		  case '\0':
		    break;
		  default:
...
textedit_main(argc, argv)
	...
	char	**argv;
{
#define	INT_TO_CADDR(v)		((caddr_t)(intptr_t)(v))
#define	GET_INT_ATTR_VAL(var)						\
	if (argc > 0) {var = INT_TO_CADDR(atoi(argv[1])); argc--, argv++;}
	...
	while ((argc--) && (attrc < ATTR_STANDARD_SIZE)) {
		...
		switch (1<<(option/2)) {
			...
			if ((argc > 0) && (argv[1][0] != '-')) {
	    		    ...
	    		    strcat(cmdline_args, " ");
			    argc--, argv++;
				textsw_attrs[attrc] = INT_TO_CADDR(!off(argv[0]));
			}
			attrc++;
			break;
