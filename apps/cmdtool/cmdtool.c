/*
 * Implements the cmdtool terminal application used by the XView app suite.
 *
	...
	char **argv;
{
	...
	if ((tmp_label1 = (char *)xv_get(base_frame, FRAME_LABEL, NULL)) == NULL) {
		(void)snprintf(frame_label, sizeof(frame_label), "%s%s%s",
			am_cmdtool ? cmd_label : shell_label,
			become_console ? console_label : " - ",
			*argv);
